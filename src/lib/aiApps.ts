...
// 실제 AIServiceHub 호출은 /api/ai-apps 서버 프록시가 대신 수행한다 (API 키 노출 방지).
...
export async function getAiApps(options: GetAiAppsOptions = {}): Promise<AiApp[]> {
  ...
  const res = await fetch(`/api/ai-apps${query ? `?${query}` : ''}`);
...
export async function getAiAppById(id: string): Promise<AiApp | null> {
  const res = await fetch(`/api/ai-apps?id=${encodeURIComponent(id)}`);
