...
export const SYSTEM_INSTRUCTIONS = {
  BASE: `
    ...
    [앱 맥락]
    이 대화와 여기서 만들어지는 결과물, 활용 방법 안내는 '클래스로그 AI' 앱의 학급 운영 흐름 안에서 이어집니다.
    학생들은 학생 페이지의 "결과" 메뉴에서 수업 결과물을 그 자료 유형에 맞는 파일 형식으로 제출·공유하고,
    교사는 주 차별 수업 활동 내용을 "기록" 메뉴를 통해 기록하며 한 주 수업을 마무리합니다.
    이 흐름을 염두에 두고, 결과물이 실제로 어떻게 제출/기록으로 이어질지 자연스럽게 고려해 대화하세요.
  `,
  SEATUK_GUIDE: `
    [세특/생기부 작성 절대 원칙]
  ...
  MATERIAL_COPILOT: `
    [역할]
    당신은 교사가 학생에게 나눠줄 학습지·유인물·활동지를 함께 만드는 'AI 코파일럿 — 수업 가이드 제작가'입니다.
  ...
  APP_GUIDE_COPILOT: `
    ...
    - 금액 관련 질문에는 [요금제 비교 데이터]에 있는 정가만 답하세요. Pro 플랜의 AI 사용량은 내부적으로 예산(크레딧) 방식으로 관리되지만, 선생님에게는 절대 달러/원 단위 사용 금액을 말하지 마세요 — "이번 달 사용 소진율 OO%"처럼 비율로만 안내하세요.
    ...
    - Pro 플랜: 횟수 제한이 아니라 넉넉한 월 예산 방식입니다. 예산을 다 쓰면 즉시 막히는 게 아니라, 더 저렴한 모델로 자동 전환되어(속도/품질이 약간 달라질 수 있음) 계속 사용할 수 있고, 그 상태에서도 아주 많이 사용하는 극소수의 경우에만 그 달 나머지 기간 동안 잠깐 제한됩니다. 즉 Pro는 "쓰다가 뚝 끊기는" 방식이 아니라 "많이 쓰면 자동으로 절약 모드로 전환"되는 방식입니다.
...
// 슬라이드 구성 개요(계획) — 화면에서 선생님이 슬라이드별로 직접 확인·수정할 수 있는 구조화된 형태
export interface SlideOutlinePlan {
  title: string;
  slides: { subtitle: string; content: string[] }[];
}
...
export async function generateSlideOutline(rawContent: string, classId?: string): Promise<SlideOutlinePlan> {
...
${sectionBlock}
[작성 규칙]
- title: 원문 내용을 대표하는 15자 내외의 슬라이드 전체 제목.
- slides: 실제 발표에 쓸 슬라이드 개수만큼 배열로 나열(특별한 요청이 없으면 5~8장 내외가 적당합니다). 각 항목은 슬라이드 한 장에 대응합니다.
  - subtitle: 그 슬라이드의 소제목.
  - content: 그 슬라이드에 들어갈 핵심 문장이나 짧은 불릿을 2~4개 정도, 원문 내용을 요약한 문자열 배열.
- 원문에 없는 내용을 지어내지 마세요.

반드시 아래 JSON 형식으로만 응답하세요 (다른 텍스트 없이):
{"title":"...","slides":[{"subtitle":"...","content":["...","..."]},...]}`;
...
export async function generateSlideDeckDraft(
  ...
  const approvedHeadings = (approvedOutline?.slides ?? [])
    .map(s => s.subtitle.trim())
    .filter(s => s.length > 0);
