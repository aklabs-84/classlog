...
export default async function handler(req: any, res: any) {
  ...
  const upstreamUrl = new URL(
    id ? `/api/public/prompts/${encodeURIComponent(String(id))}` : '/api/public/prompts',
    baseUrl
  );
  if (!id) {
  ...
  try {
    ...
    console.error('[api/ai-prompts] error:', error?.message);
    return res.status(500).json({ error: 'AIServiceHub 프롬프트 조회 중 오류가 발생했습니다.' });
  }
}
